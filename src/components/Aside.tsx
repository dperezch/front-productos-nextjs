...
const Aside = () => {
  ...
  return (
    ...
    <aside id="default-sidebar" className={`fixed top-0 left-0 z-40 w-64 h-screen transition-transform -translate-x-full sm:translate-x-0 ${drawer? 'transform-none': ''} border-r border-gray-200 `} aria-label="Sidebar">
        <div className="h-full px-3 py-4 overflow-y-auto bg-white dark:bg-gray-800 flex flex-col">
            ...
            <ul className="space-y-4 font-medium">
                ...
                <li className={`${session? "": "hidden" }`}>
                    <Link href="/" onClick={() => signOut()} className="flex items-center p-2 text-red-500 rounded-lg dark:text-white hover:text-red-600 hover:bg-gray-100 dark:hover:bg-gray-700 group ">
                        <svg className="flex-shrink-0 w-5 h-5 text-red-500 transition duration-75 dark:text-gray-400 group-hover:text-red-600 dark:group-hover:text-white" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 16 16">
